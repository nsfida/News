...
const els = {
  ...
  downloadInstallmentsPdfBtn: document.getElementById("downloadInstallmentsPdfBtn"),
  ...
  searchInstallments: document.getElementById("searchInstallments"),
...
function normalizeInstallmentNote(noteValue, addTag){
  const text = String(noteValue || "").trim();
  if (addTag && !text.includes(INSTALLMENT_TAG)) {
    return `${INSTALLMENT_TAG} ${text}`.trim();
  }
  return text;
}
...
function renderLoanCards(container, direction, searchKey = direction, options = {}){
      ...
      <details class="loan">
          ...
          <div class="loan-top">
            <div class="lt-main">
              ...
              <div class="loan-sub">
                ...
                <span>${escapeHtml(`${group.groupCount || 1} plan${(group.groupCount || 1) > 1 ? "s" : ""}`)}</span>
                <span class="badge blue">Installment</span>
            ...
            <div class="lt-action">
              <div class="menu-wrap">
                ...
                <div class="menu-dropdown" data-person-menu-panel="${escapeHtml(group.primaryGroupId || group.person_name || "menu")}">
                  ...
                  <button class="menu-item personActionBtn" type="button" data-action="move-taken" data-person="${encodeURIComponent(group.person_name || "")}" data-direction="${escapeHtml(direction)}">Move to Taken</button>
        ...
        <div class="detail">
          <div class="detail-head">
            <div>
              <h4>Timeline</h4>
              <p>Oldest to newest inside each plan. New activity still brings the plan card to the top.</p>
            </div>
  ...
  container.querySelectorAll(".personActionBtn").forEach(btn => btn.addEventListener("click", async e => {
    ...
    } else if (action === "move-taken") {
      await movePersonToTaken(person, dir);
    }
  }));
...
function openEntryModal(modalType, direction){
  ...
  if (modalType === "principal"){
    title.textContent = "New Installment Plan";
    desc.textContent = "Record a new installment plan taken from someone.";
    els.principalModalForm.classList.remove("hide");
  } else if (modalType === "payment"){
    title.textContent = "Installment Payment";
    desc.textContent = "Record an installment payment returned back.";
...
function populateLoanSelect(direction){
  const installmentEntries = state.entries.filter(e => 
    e.direction === direction && hasInstallmentTag(e.notes)
  );
  const groups = groupByLoan(installmentEntries);
  ...
  els.modalLoanSelect.innerHTML = `<option value="">Select an installment plan...</option>${options}`;
}
...
async function savePrincipalEntry(formData){
  ...
  const entry = {
    ...
    loan_date: loanDate,
    action_date: null,
    notes: normalizeInstallmentNote(notes, true)
  };
...
async function savePaymentEntry(formData){
  ...
  const installmentEntries = state.entries.filter(e => 
    e.direction === state.modalDirection && hasInstallmentTag(e.notes)
  );
  const groups = groupByLoan(installmentEntries);
...
async function saveEditEntry(formData){
  ...
  entry.notes = hasInstallmentTag(entry.notes) 
    ? normalizeInstallmentNote(entry.notes, true)
    : notes;
...
async function deleteEntry(id){
  ...
  if (entry.entry_kind === "principal"){
    if (!confirm(`Delete the entire installment plan for ${entry.person_name}? This will also remove ALL linked payments.`)) return;
...
async function deletePersonRecords(personNameEncoded, direction){
  ...
  if (!recordsCount) {
    alert("No installment records found for this person.");
    return;
  }

  const directionLabel = direction === "given" ? "given" : "taken";
  if (!confirm(`Delete full installment record for ${personName}? This will remove ${recordsCount} entr${recordsCount === 1 ? "y" : "ies"} from ${directionLabel}.`)) return;
...
async function movePersonToTaken(personNameEncoded, direction){
  const personName = decodeURIComponent(personNameEncoded || "").trim();
  ...
  if (!confirm(`Move ${personName} to regular loan tracking? This will remove the installment tag and move the records to the Taken section.`)) return;

  state.entries = state.entries.map(entry => (
    entry.direction === "taken" && 
    String(entry.person_name || "").trim() === personName &&
    hasInstallmentTag(entry.notes)
      ? { ...entry, notes: entry.notes.replace(INSTALLMENT_TAG, "").trim() }
      : entry
  ));
  updateSessionStorage();
  renderInstallments();
  ...
  // Redirect to taken page
  window.location.href = "/pages/taken/index.html";
}
...
function renderInstallments(){
  const installmentEntries = state.entries.filter(e => hasInstallmentTag(e.notes));
  renderLoanCards(els.installmentsList, "taken", "installments", {
    groupFilter: (group) => {
      return group.entries.some(e => hasInstallmentTag(e.notes));
    }
  });
}
...
function bindEvents(){
  ...
  // Search
  els.searchInstallments.addEventListener("input", (e) => {
    state.search.installments = e.target.value;
    renderInstallments();
  });
  
  // Filters
  document.querySelectorAll('[data-filter="installments"]').forEach(radio => {
    radio.addEventListener("change", (e) => {
      state.statusFilter.installments = e.target.value;
      renderInstallments();
    });
  });
  
  document.querySelectorAll('[data-currency-filter="installments"]').forEach(radio => {
    radio.addEventListener("change", (e) => {
      state.currencyFilter.installments = e.target.value;
      renderInstallments();
    });
  });
  ...
  document.querySelectorAll('[data-open-modal="principal"]').forEach(btn => {
    btn.addEventListener("click", () => openEntryModal("principal", "taken"));
  });
  
  document.querySelectorAll('[data-open-modal="payment"]').forEach(btn => {
    btn.addEventListener("click", () => openEntryModal("payment", "taken"));
  });
  ...
  els.downloadInstallmentsPdfBtn.addEventListener('click', () => {
    alert('PDF download functionality to be implemented');
  });
