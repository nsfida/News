...
function renderNews(data){
  ...
  data.forEach(item=>{
    ...
    card.innerHTML=`<div class="news-image"><img src="${images[0]}" class="slider-img"></div><div class="news-content"><div class="news-category">${item.category}</div><div class="news-title">${item.title}</div><div class="news-date">${item.date}</div></div>`;
    if(images.length>1){
      let index=0;
      const imgEl=card.querySelector(".slider-img");
      setInterval(()=>{index=(index+1)%images.length;imgEl.style.opacity=0;setTimeout(()=>{imgEl.src=images[index];imgEl.style.opacity=1;},200);},1500);
    }
...
function openExpanded(item){
  ...
  if(images.length>1){
    setInterval(()=>{index=(index+1)%images.length;expandedImage.src=images[index];},2000);
  }
  expandedContent.innerText=item.content;
  expandedSection.classList.add("active");
  document.body.style.overflow="hidden";
  window.history.replaceState({}, "", `?news=${item.id}`);
}

function closeExpandedView(){expandedSection.classList.remove("active");document.body.style.overflow="auto";}
closeExpanded.addEventListener("click",closeExpandedView);
window.addEventListener("click",e=>{if(e.target===expandedSection) closeExpandedView();});

searchInput.addEventListener("input",e=>{
  const value=e.target.value.toLowerCase();
  renderNews(newsData.filter(n=>n.title.toLowerCase().includes(value)||n.category.toLowerCase().includes(value)||n.content.toLowerCase().includes(value)));
});
...
navItems.forEach(item=>{
  item.addEventListener("click",()=>{
    ...
    renderNews(category==="all"?newsData:newsData.filter(n=>n.category.toLowerCase()===category));
  });
});

function renderBreaking(data){
  breakingTicker.innerHTML="";
  data.slice(0,10).forEach(n=>{const span=document.createElement("span");span.innerText=n.title;breakingTicker.appendChild(span);});
}

window.addEventListener("scroll",()=>{if(window.innerHeight+window.scrollY>=document.body.offsetHeight-100) loadMoreNews();});
setInterval(fetchNews,120000);
fetchNews();

const projectsToggle=document.getElementById("projectsToggle");
const projectsDropdown=document.getElementById("projectsDropdown");
projectsToggle.addEventListener("click",()=>{projectsDropdown.classList.toggle("active");});
window.addEventListener("click",(e)=>{if(!e.target.closest(".projects-menu-wrapper")) projectsDropdown.classList.remove("active");});
...
function openNewsFromURL(){
  ...
  if(newsItem) openExpanded(newsItem);
}
