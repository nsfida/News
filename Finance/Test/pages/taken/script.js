...
const els = {
  ...
  searchTaken: document.getElementById("searchTaken"),
            ...
            <div class="lt-action">
              <div class="menu-wrap">
                ...
                <div class="menu-dropdown" data-person-menu-panel="${escapeHtml(group.primaryGroupId || group.person_name || "menu")}">
                  ...
                  <button class="menu-item personActionBtn" type="button" data-action="move-installment" data-person="${encodeURIComponent(group.person_name || "")}" data-direction="${escapeHtml(direction)}">Move to Installments</button>
...
async function movePersonToInstallments(personNameEncoded, direction){
  ...
  window.location.href = "/pages/installments/index.html";
}

function normalizeInstallmentNote(note, addTag){
  const text = String(note || "").trim();
  const installmentTag = "[INSTALLMENT]";
  if (addTag && !text.includes(installmentTag)) {
    return `${installmentTag} ${text}`.trim();
  }
  return text;
}
...
function bindEvents(){
  ...
  // Search
  els.searchTaken.addEventListener("input", (e) => {
    state.search.taken = e.target.value;
    renderTaken();
  });
  
  // Filters
  document.querySelectorAll('[data-filter="taken"]').forEach(radio => {
    radio.addEventListener("change", (e) => {
      state.statusFilter.taken = e.target.value;
      renderTaken();
    });
  });
  
  document.querySelectorAll('[data-currency-filter="taken"]').forEach(radio => {
    radio.addEventListener("change", (e) => {
      state.currencyFilter.taken = e.target.value;
      renderTaken();
    });
  });
  ...
  els.downloadTakenPdfBtn.addEventListener('click', () => {
    alert('PDF download functionality to be implemented');
  });
