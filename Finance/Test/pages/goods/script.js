...
const els = {
  ...
  downloadGoodsPdfBtn: document.getElementById("downloadGoodsPdfBtn"),
  ...
  modalGoodsSelect: document.getElementById("modalGoodsSelect"),
  editModal: document.getElementById("editModal"),
  editForm: document.getElementById("editForm"),
  searchGoods: document.getElementById("searchGoods"),
...
function normalizeGoodsNote(noteValue, addTag){
  const text = String(noteValue || "").trim();
  if (addTag && !text.includes(GOODS_TAG)) {
    return `${GOODS_TAG} ${text}`.trim();
  }
  return text;
}
...
function renderLoanCards(container, direction, searchKey = direction, options = {}){
  ...
  if (!groups.length){
    container.innerHTML = `<div class="empty">No goods transactions found.</div>`;
    return;
  }
      ...
      <details class="loan">
          ...
          <div class="loan-top">
            <div class="lt-main">
              ...
              <div class="loan-sub">
                ...
                <span>${escapeHtml(`${group.groupCount || 1} item${(group.groupCount || 1) > 1 ? "s" : ""}`)}</span>
                <span class="badge blue">Goods</span>
                ${openOnly ? '<span class="badge orange">Unsold</span>' : '<span class="badge green">Sold</span>'}
              </div>
            </div>
            <div class="cell lt-status"><small>Status</small><strong><span class="badge ${statusClass}">${escapeHtml(group.status)}</span></strong></div>
            <div class="cell lt-principal"><small>Purchase Price</small><strong>${money(group.principalTotal, group.currency)}</strong></div>
        ...
        <div class="detail">
          <div class="detail-head">
            <div>
              <h4>Timeline</h4>
              <p>Oldest to newest inside each item. New activity still brings the item card to the top.</p>
            </div>
              ...
              <tbody>
                ${group.rows.map((row, index) => `
                  <tr>
                    ...
                    <td><span class="badge ${row.kind === "principal" ? "blue" : row.kind === "partial" ? "orange" : "green"}">${row.kind === "principal" ? "Purchased" : row.kind === "partial" ? "Partial Sale" : "Sold"}</span></td>
...
function openGoodsModal(modalType, direction){
  state.modalDirection = direction;
  const modal = els.goodsModal;
  const title = els.goodsModalTitle;
  const desc = els.goodsModalDesc;
  
  // Hide all forms first
  els.goodsBoughtForm.classList.add("hide");
  els.goodsSoldForm.classList.add("hide");
  
  if (modalType === "principal"){
    title.textContent = "Goods Bought";
    desc.textContent = "Record a new goods purchase.";
    els.goodsBoughtForm.classList.remove("hide");
  } else if (modalType === "payment"){
    title.textContent = "Goods Sold";
    desc.textContent = "Record a goods sale.";
    els.goodsSoldForm.classList.remove("hide");
    populateGoodsSelect(direction);
  }
...
function populateGoodsSelect(direction){
  const goodsEntries = state.entries.filter(e => 
    e.direction === direction && hasGoodsTag(e.notes)
  );
  const groups = groupByLoan(goodsEntries);
  ...
  els.modalGoodsSelect.innerHTML = `<option value="">Select a purchased item...</option>${options}`;
}

function closeGoodsModal(){
  els.goodsModal.classList.add("hide");
  els.goodsModal.setAttribute("aria-hidden", "true");
...
async function saveGoodsBought(formData){
  const personName = formData.get("item_name").trim();
  ...
  const groupId = `goods_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
  ...
  const entry = {
    ...
    loan_date: loanDate,
    action_date: null,
    notes: normalizeGoodsNote(notes, true)
  };
...
async function saveGoodsSold(formData){
  ...
  if (!groupId) {
    alert("Please select a purchased item.");
    return false;
  }
  ...
  const goodsEntries = state.entries.filter(e => 
    e.direction === state.modalDirection && hasGoodsTag(e.notes)
  );
  const groups = groupByLoan(goodsEntries);
...
async function saveEditEntry(formData){
  ...
  entry.notes = hasGoodsTag(entry.notes) 
    ? normalizeGoodsNote(entry.notes, true)
    : notes;
...
async function deleteEntry(id){
  ...
  if (entry.entry_kind === "principal"){
    if (!confirm(`Delete the entire goods record for ${entry.person_name}? This will also remove ALL linked sales.`)) return;
...
async function deletePersonRecords(personNameEncoded, direction){
  ...
  if (!recordsCount) {
    alert("No goods records found for this person.");
    return;
  }

  const directionLabel = direction === "given" ? "given" : "taken";
  if (!confirm(`Delete full goods record for ${personName}? This will remove ${recordsCount} entr${recordsCount === 1 ? "y" : "ies"} from ${directionLabel}.`)) return;
...
async function renamePersonRecords(personNameEncoded, direction){
  ...
  const nextName = prompt("Enter new item name:", currentName);
  ...
  state.entries = state.entries.map(entry => (
    ...
    String(entry.person_name || "").trim() === currentName &&
...
function renderGoods(){
  const goodsEntries = state.entries.filter(e => hasGoodsTag(e.notes));
  renderLoanCards(els.goodsList, "taken", "goods", {
    groupFilter: (group) => {
      return group.entries.some(e => hasGoodsTag(e.notes));
    }
  });
}
...
function bindEvents(){
  ...
  // Search
  els.searchGoods.addEventListener("input", (e) => {
    state.search.goods = e.target.value;
    renderGoods();
  });
  
  // Filters
  document.querySelectorAll('[data-filter="goods"]').forEach(radio => {
    radio.addEventListener("change", (e) => {
      state.statusFilter.goods = e.target.value;
      renderGoods();
    });
  });
  
  document.querySelectorAll('[data-currency-filter="goods"]').forEach(radio => {
    radio.addEventListener("change", (e) => {
      state.currencyFilter.goods = e.target.value;
      renderGoods();
    });
  });
  ...
  document.querySelectorAll('[data-open-modal="principal"]').forEach(btn => {
    btn.addEventListener("click", () => openGoodsModal("principal", "taken"));
  });
  
  document.querySelectorAll('[data-open-modal="payment"]').forEach(btn => {
    btn.addEventListener("click", () => openGoodsModal("payment", "taken"));
  });
  
  // Modal close buttons
  document.querySelectorAll('[data-close-modal="goodsModal"]').forEach(btn => {
    btn.addEventListener("click", closeGoodsModal);
  });
  ...
  document.querySelectorAll('.modal-backdrop').forEach(backdrop => {
    backdrop.addEventListener('click', (e) => {
      if (e.target === backdrop) {
        ...
        if (modalId === 'goodsModal') closeGoodsModal();
  ...
  els.goodsBoughtForm.addEventListener("submit", async (e) => {
    e.preventDefault();
    await saveGoodsBought(new FormData(e.target));
  });
  
  els.goodsSoldForm.addEventListener("submit", async (e) => {
    e.preventDefault();
    await saveGoodsSold(new FormData(e.target));
  });
  ...
  els.downloadGoodsPdfBtn.addEventListener('click', () => {
    alert('PDF download functionality to be implemented');
  });
